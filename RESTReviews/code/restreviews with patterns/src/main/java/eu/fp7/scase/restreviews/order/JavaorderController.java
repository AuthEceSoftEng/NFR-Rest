...
public class JavaorderController{
    ...
    private UriInfo oApplicationUri;
	    ...
	    public JavaorderModel putorder(@PathParam("accountId") int accountId, @PathParam("orderId") int orderId,  JavaorderModel oJavaorderModel, @DefaultValue("keepMemento") @QueryParam("action") String action, @DefaultValue("0") @QueryParam("mNum") int mementoNum){
	        if(action.equalsIgnoreCase("keepMemento")){
				...
				PutorderHandler oPutorderHandler = new PutorderHandler(accountId, oJavaorderModel.getorderId(), oJavaorderModel, oApplicationUri);
		...
		@DELETE
		@Produces("application/JSON")
	    public JavaorderModel deleteorder(@PathParam("orderId") int orderId,@DefaultValue("yes") @QueryParam("keepMemento") String keepMemento){
			if(keepMemento.equalsIgnoreCase("yes")){
				JavaorderMemento oMemento = new JavaorderMemento();
				oMemento.createMemento();
				oMemento.defineMemento(orderId);
			}        
...
/*
 ...
 does it have builder pattern?	DesignPatternsLayerPSM.impl.JavaBuilderPatternImpl@44af7771
